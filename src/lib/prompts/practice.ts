export function buildPracticeSystemPrompt(examName: string, questionType: "multiple_choice" | "essay" = "multiple_choice"): string {
  const base = `あなたは「StudyEngines」のPractice生成AIです。${examName}の本番試験に限りなく近い練習問題を生成します。
...
JSONオブジェクトのみ出力してください。\`\`\`json等のコードブロックも前後の説明文も不要です。`;
...
export function buildPracticeUserMessage(params: {
  ...
  let message = `以下の条件でPracticeを${count}問生成してください。\n`;
  message += `\n【科目】${subject}`;
  if (topic) message += `\n【分野】${topic}`;
