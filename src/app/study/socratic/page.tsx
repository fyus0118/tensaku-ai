...
function SocraticContent() {
  ...
  useEffect(() => {
    if (phase === "session" && selectedSubject && !started) {
      setStarted(true);
      fetchResponse(`${selectedSubject.name}${selectedTopic ? `の${selectedTopic}` : ""}について、考えさせる問いを1つ投げてください。`);
    }
  ...
  const startSession = (subject: ExamSubject, topic?: string) => {
    ...
    setStarted(false);
    setPhase("session");
    router.replace(`/study/socratic?exam=${examId}&subject=${encodeURIComponent(subject.name)}${topic ? `&topic=${encodeURIComponent(topic)}` : ""}`);
  };
  ...
  if (phase === "select") return (
    <main className="min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        <div className="mb-8">
          <h2 className="text-xl font-bold mb-2">何について考えますか？</h2>
          <p className="text-sm text-[var(--color-text-secondary)]">AIは答えを教えません。問いだけ投げて、あなたに考えさせます。</p>
        </div>
                ...
                <div className="px-6 pb-4 flex flex-wrap gap-2">
                  {subject.topics.map(topic => (
                    <button key={topic} onClick={() => startSession(subject, topic)}
                      className="px-3 py-1.5 rounded-lg bg-[var(--color-bg-secondary)] hover:bg-violet-500/10 hover:text-violet-400 text-xs text-[var(--color-text-secondary)] transition-colors">{topic}</button>
                  ))}
                </div>
              )}
            </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto px-6 py-6">
          ...
          {loading && !streamingText && (
            <div className="mb-6 flex gap-3">
              <div className="w-8 h-8 rounded-lg bg-violet-500/10 flex items-center justify-center shrink-0"><Loader2 className="w-4 h-4 text-violet-400 animate-spin" /></div>
        ...
        <form onSubmit={handleSubmit} className="max-w-3xl mx-auto px-6 py-4">
          <div className="flex gap-3 items-end">
            <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
              placeholder="考えを入力... (Shift+Enterで改行)" rows={1}
