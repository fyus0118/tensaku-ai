  ...
  return (
    <main className="min-h-screen flex flex-col">
      ...
      <header className="border-b border-[var(--color-border)] shrink-0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          ...
          {result && (
            <button
              onClick={handleReset}
              className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg border border-[var(--color-border)] hover:border-[var(--color-border-hover)] text-sm text-[var(--color-text-secondary)] transition-colors"
            >
              <RotateCcw className="w-4 h-4" />
              <span className="hidden sm:inline">新しい添削</span>
            </button>
          )}
        </div>
      </header>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        {/* Left: Input — モバイルでは結果がない時のみフル表示、結果がある時は縮小 */}
        <div className={`lg:w-1/2 border-b lg:border-b-0 lg:border-r border-[var(--color-border)] flex flex-col ${result ? "max-h-[40vh] lg:max-h-none" : "flex-1"}`}>
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
            {/* Settings */}
            <div className="p-4 sm:p-6 border-b border-[var(--color-border)] space-y-3 sm:space-y-4 overflow-y-auto shrink-0">
              {isEssay ? (
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2.5 sm:py-2 rounded-lg bg-[var(--color-bg-card)] border border-[var(--color-border)] text-sm focus:border-[var(--color-accent)] focus:outline-none"
                      >
                        <option>小論文</option>
                        <option>志望理由書</option>
                        <option>自己推薦書</option>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2.5 sm:py-2 rounded-lg bg-[var(--color-bg-card)] border border-[var(--color-border)] text-sm placeholder:text-[var(--color-text-muted)] focus:border-[var(--color-accent)] focus:outline-none"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-[var(--color-text-secondary)] mb-1">
            ...
            {/* Textarea */}
            <div className="flex-1 flex flex-col p-4 sm:p-6 overflow-hidden min-h-[200px]">
              ...
              {/* Bottom bar */}
              <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-[var(--color-border)] shrink-0">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 sm:px-6 py-2.5 rounded-xl bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-white font-bold text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? (
                    <>
        ...
        {/* Right: Result */}
        <div className={`lg:w-1/2 flex flex-col overflow-hidden ${result ? "flex-1" : ""}`}>
